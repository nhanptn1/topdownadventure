...
3n. **Playtest the new crafting system** (see §0m) — open the inventory, hit the Craft tab, confirm all 9 recipes list with correct have/need material counts, that the button disables when short on a material, and that crafting deducts materials and produces the item correctly (including the equip-upgrade prompt if it's better than what's equipped). Then gauge whether the recipe costs (`ItemDatabase.CRAFTING_RECIPES`) feel right against actual material farming rate -- they're a first-pass guess.
