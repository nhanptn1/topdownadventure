...
*Written 2026-07-10 as a resumption reference, updated same-day after an auto-attack bugfix session, then again after adding a New Game+ postgame loop, then again after adding NG+-exclusive mythic gear, then again after the entire weapon/armor/accessory roster was replaced with new art and items, then again after gear drops got randomized per-drop stat rolls, then again after adding a character skill system (active skill + passive per character), then again after the skill system got level-gating, a passive leveling curve, base defense, and auto-cast, then again after the active skill got its own visual effect, then again after a full player/enemy/boss balance review, then again after fixing overlapping/clipped stat text in the inventory UI, then again after fixing the skill effect landing near the player instead of the enemy, then again (2026-07-14) after a bug-fix pass covering skill auto-cast firing at nothing, the final boss's unreachable Phase 1 melee attack, a one-shot-only victory popup, and a rework of gear stat rolls to per-copy instances, then again same day after New Game+ was changed to actually reset level/gear instead of carrying everything over, then again (2026-07-17) after phase 1 was marked done and a phase 2 crafting system shipped as the first phase 2 feature, then again same day after a ranged "Skeleton Archer" enemy variant shipped as the second phase 2 feature, then again same day after a new required map ("Frostbound Wastes") was inserted between map2 and map3 as the third phase 2 feature, then again same day after a phase-2 review fixed New Game+ wiping crafting materials along with everything else, then again same day after the crafting system was rebalanced to epic+mythic only with a higher material drop rate. This project has been dormant while work continued on the sibling "Endless Archer RPG" project — read this before touching TopDownAdventure again.*

## 0q. Session (2026-07-17): crafting rebalanced -- epic+mythic only, higher material drop rate

The user found material drops too rare to make crafting feel usable, and asked to restrict crafting to epic-tier and above (dropping common/rare, which are common enough from regular drops that crafting them added little) -- "epic and above" includes mythic, which previously had no recipe at all.

- **`ItemDatabase.CATEGORY_WEIGHTS`**: `material` bumped 0.05 -> 0.20 (taken from `consumable`, 0.40 -> 0.25; `gear`'s 0.55 untouched so the core loot loop isn't affected) -- a 4x increase in material drop rate, first-pass number like everything else here.
- **`ItemDatabase.CRAFTING_RECIPES`**: the 6 common/rare recipes removed; the 3 epic recipes kept unchanged; **3 new mythic recipes added** (`sovereigns_bow`/`sovereigns_aegis`/`sovereigns_signet`, each `{iron_ingot: 10, ruby_gem: 8, dragon_scale: 6}`, roughly 1.6x the epic cost).
- **Mythic recipes are NG+-gated**, confirmed with the user rather than assumed: `player.gd`'s `craft_item()` now also checks `ItemDatabase.get_item(item_id).get("rarity","") == "mythic"` and refuses to craft unless `GlobalState.ng_plus_level > 0` -- mirrors mythic gear's existing drop-only NG+ exclusivity (`MYTHIC_DROP_CHANCE`) instead of letting crafting bypass it. `inventory_ui.gd`'s `_build_craft_row()` mirrors the same check: a locked mythic recipe still *shows* in the Craft tab (so players know it exists as a reason to start NG+) but its cost line reads "Requires New Game+" and the Craft button stays disabled regardless of material counts.
- **Not yet manually playtested** -- confirm materials now drop noticeably more often, that the Craft tab only lists 6 recipes (3 epic + 3 mythic) instead of 9, that mythic recipes show locked pre-NG+ and become craftable after starting NG+ with enough materials.

## 0p. Session (2026-07-17): phase 2 review -- crafting materials now survive New Game+

The user asked for a review pass over phase 2 so far and flagged one real gap: §0l's New Game+ reset called `storage.clear()` unconditionally, which wiped crafting materials (`dragon_scale`/`iron_ingot`/`ruby_gem`) right alongside consumables and quest items -- meaning every NG+ cycle forced re-farming materials from zero for §0m's crafting system, with no real difficulty-curve purpose (unlike gear/level, a material stockpile doesn't trivialize the harder NG+ run the way carried-over gear would).

- **`GlobalState.start_new_game_plus()`** now filters `storage` down to only entries whose `ItemDatabase.get_item(item_id).get("item_type", "")` is `"material"` before clearing the rest, instead of blanket-clearing. Consumables and quest items (e.g. `ancient_relic`) still reset along with gear/level, matching the original NG+ design intent -- only materials are the deliberate exception.
- Reviewed the rest of phase 2 (crafting, the ranged Skeleton Archer, Frostbound Wastes) for other issues while in there; found nothing else wrong.
- **Not yet manually playtested** -- pick up some materials, beat the final boss, start NG+, open the inventory and confirm the material counts survived while consumables/quest items/gear are all gone.

## 0o. Session (2026-07-17): new required map "Frostbound Wastes" (map2b.tscn), phase 2 feature 3

Third phase 2 feature. The user picked "new map/side-area" from the "more content" menu, then specifically chose a new **required** main-path map (not an optional side-area like the existing Cave), gated behind map2. Progression is now `map.tscn` -> `map2.tscn` -> **`map2b.tscn`** -> `map3.tscn` -> `map4.tscn` (final boss stage).

- **Only change to an existing file**: `map2.tscn`'s existing exit `Gate` (`required_level = 20`, `required_boss_id = "gate2_boss"` i.e. Gate Overlord, unchanged) had its `next_map_path` repointed from `map3.tscn` to `map2b.tscn`. Nothing else about map2 changed.
- **New `map2b.tscn`** ("Frostbound Wastes") bridges a real gap in the existing curve: map2's own mobs top out at level 17 but map3's start at 23, and Gate Overlord (lv25) to Dragon Sovereign (lv32) was a big single jump for a gate boss. Reuses the exact same wall/collision boilerplate as every other main map (1920x1200), `tile_grass.png` tinted icy pale-blue (`Color(0.72, 0.82, 0.95, 1)`) to read as frozen tundra distinct from map2/map3's shared plain `tile_dirt.png` -- the same modulate-tint-an-existing-tile technique `cave.tscn` already used. Decor is corner `decor_mountain` x4, 10 scattered `decor_rock`, 4 `decor_tree_dead` -- deliberately no bush/flowers/grass-tuft, too lush for a wasteland.
  - Two regular spawners (`dragon_spawner.gd`, still the generic spawner script despite its name), levels 18-22, `map_tier = 3`: melee "Frozen Zombie" (`zombie` species, pale-blue tint) and a **ranged** "Frost Drake" (`dragon` species, `is_ranged = true` -- the very mechanic added in §0n, reused immediately for a second, differently-themed ranged mob).
  - New gate boss "Frostbound Warden" (`skeleton` species, direct `enemy.tscn` instance like every other gate boss, not spawner-based), level 29 -- interpolated bridge point between Gate Overlord (25) and Dragon Sovereign (32) -- with `bonus_max_health`/`bonus_attack_damage` (3185/60) linearly interpolated between those two bosses' own values at the same ratio. `gate_boss_id = "gate2b_boss"`, a brand-new id -- no collision with `gate2_boss`/`gate3_boss`, and `GlobalState.start_new_game_plus()` already clears `defeated_gate_bosses` wholesale (not per-id), so NG+ needed zero changes for the new id to work.
  - Its own exit `Gate` (`required_level = 22`, `required_boss_id = "gate2b_boss"`) leads into the existing, completely untouched `map3.tscn`.
- **No script changes needed at all** -- confirmed `gate.gd`/`dragon_spawner.gd`/`enemy.gd` are already fully data-driven via `@export`s (including `is_ranged` from §0n), and grepped the whole `scripts/` tree to confirm nothing hardcodes `map2.tscn`/`map3.tscn`/`map4.tscn` paths outside each gate's own `next_map_path` value -- `minimap_ui.gd`/`character_select.gd` are both map-path-agnostic.
- **All level/stat numbers are a first-pass guess**, flagged as the knob to retune same as every other balance constant in this project.
- **Not yet manually playtested** -- reach map2's exit gate and confirm it now leads to Frostbound Wastes instead of straight to map3; fight through Frozen Zombie/Frost Drake; defeat the Frostbound Warden and confirm its own exit gate correctly leads into the existing map3; eyeball whether the icy tint actually reads as visually distinct from its neighbors at a glance.

## 0n. Session (2026-07-17): ranged "Skeleton Archer" enemy variant (phase 2, feature 2)

Second phase 2 feature, picked from a "more content" menu (enemies/maps/2nd active skill/gear tiers) then narrowed down to "new enemy species." Checking `assets/sprites` first turned up only 4 real enemy sprite sets in the whole project (dragon/slime/zombie/skeleton) -- every existing "new" enemy (Ancient Dragon, Plague Zombie, the Dragon Sovereign/Gate Overlord guardians) is actually one of those 4 reskinned via `body_tint` + a new `animal_name` + stat bonuses, not new art. Since generating new pixel art isn't something this assistant can do, and the user confirmed reusing that same reskin convention (rather than waiting on a new reference sheet) was fine as long as the *behavior* was genuinely new, not just the color.

- **`enemy.gd`**: new `is_ranged`/`ranged_attack_range` (220px)/`ranged_projectile_speed` (220) exports. In `CHASE` state, a ranged enemy now holds at `ranged_attack_range` instead of always closing to melee and fires `_try_ranged_attack()` on a `RANGED_ATTACK_COOLDOWN` (1.6s) -- reuses `boss_projectile.tscn`/`boss_projectile.gd` exactly as `ultimate_boss.gd`'s Phase 2+ ranged bolt already does (direction/speed/damage/global_position, straight `body_entered` vs. the player's physics body, no Hurtbox needed). If the player closes the gap into melee range anyway, the existing `AttackArea` contact-damage path is untouched and still fires -- ranged enemies aren't defenseless up close, they just prefer distance. `_physics_process`'s unused `_delta` param is now used (ticks `_ranged_cooldown_remaining`).
- **`dragon_spawner.gd`** (the generic enemy-spawner script despite its name): forwards the same three new exports to each spawned `enemy.tscn` instance, matching how every other per-enemy field already gets forwarded.
- **New spawner**: `map2.tscn`'s `SkeletonArcherSpawner`, alongside the existing `DragonSpawner`/`SkeletonSpawner` -- reuses the `skeleton` sprite with a pale-green `body_tint` (`Color(0.55, 0.8, 0.55, 1)`) to read as visually distinct from the plain-white regular Skeleton, same level range (12-17) and `map_tier` (2) as its neighbors, `dragon_count = 4` (lighter than the melee Skeleton's 6, since a kiting ranged mob covers more effective threat area per unit).
- **Damage/cooldown numbers are a first-pass guess**, flagged as the knob to retune same as everything else -- `ranged_attack_range`/`RANGED_ATTACK_COOLDOWN` on the spawner/script if it feels too easy or too oppressive to fight at range.
- **Not yet manually playtested** -- confirm a Skeleton Archer actually holds distance and fires instead of closing to melee, that its shot visibly matches the final boss's teal bolt look, that closing the gap yourself still triggers normal melee contact damage, and that it doesn't get stuck refusing to fire if a wall/decor blocks line of sight at its hold distance (no LOS re-check was added specifically for the ranged hold-and-fire behavior -- it inherits whatever the existing FOV/LOS aggro check already established when it entered CHASE).

## 0m. Session (2026-07-17): phase 1 marked done; phase 2 begins with a crafting system

The user marked phase 1 (the base game) complete and asked for phase 2 features. Auditing the current state for a natural starting point turned up a concrete, already-planted gap: `item_database.gd`'s three material items (`dragon_scale`/`iron_ingot`/`ruby_gem`) all say "useful for future crafting" in their description text, and `ancient_relic` says "someone might want this" -- but no crafting system, NPC, vendor, or quest turn-in existed anywhere in the codebase. Materials were pure flavor items with zero gameplay use. The user picked "crafting system" from a shortlist of phase 2 directions (the others: elemental status effects, NG+ meta-progression currency, more content/enemies/maps -- worth keeping in mind for later phase 2 work).

- **Recipes are deterministic and item-specific**, not more RNG on top of RNG: `ItemDatabase.CRAFTING_RECIPES` (`item_id -> {material_id: count}`) lets a player spend a fixed material bundle to craft a *specific* named gear item by id, covering all 9 non-mythic weapon/armor/accessory items (common: 2x iron_ingot; rare: 4x iron_ingot + 3x ruby_gem; epic: 6x iron_ingot + 5x ruby_gem + 4x dragon_scale). **Mythic gear is deliberately not craftable** -- it stays the NG+-exclusive drop-only reward established in §0b, and crafting doesn't undermine that. `ancient_relic` is untouched -- it's a trophy/proof-of-defeat item, not a crafting input; a full NPC/quest-turn-in flow for it would be a separate, bigger feature.
- **`player.gd`'s new `craft_item(item_id)`** checks the recipe's materials against `GlobalState.storage`, deducts them via the existing `storage_remove()`, then calls the existing `add_item_to_inventory()` unchanged -- crafted gear rolls its own independent per-copy stats via `GlobalState.gear_add()` and pops the equip-upgrade prompt exactly like a dropped item would, no special-casing needed.
- **UI**: added a "Craft" tab to the existing inventory panel rather than building any new NPC/dialogue scaffolding (none exists in this project). `inventory_ui.tscn`'s existing Quick Slots/Inventory sections were re-parented under a new `BagSection`, with a sibling `CraftSection` (hidden by default) added alongside it; a new `CraftToggleButton` in the title bar flips between them. Each craft row shows the item name/rarity, a live `MaterialName have/need` cost breakdown, and a `Craft` button that disables itself if any material is short. Opening the inventory always resets to the Bag tab.
- **Recipe costs are a first-pass guess**, flagged as the knob to retune: materials only drop from regular (non-guardian) enemies via `roll_random_drop()` at a flat 5% category chance split 3 ways (~1.7% per material type per kill), independent of map tier, so farming rate is uniform throughout the game and wasn't otherwise tuned against these specific recipe costs.
- **Not yet manually playtested** -- open the inventory, hit Craft, confirm the recipe list and have/need counts render correctly, disabled buttons work when short on materials, and that crafting actually deducts materials and produces the item (with the equip-upgrade prompt firing if applicable).
...
3m. **Playtest NG+'s reset** (see §0l, amended §0p) — start New Game+, confirm level/gear/consumables/quest items are actually gone and the character starts at level 1 with base stats only, **but that crafting material counts survived intact**, and play at least the first couple of map-1 fights to gauge whether `difficulty_multiplier()`'s scaling (originally tuned against an overleveled, fully-geared NG+ player) is still fair for a fresh level-1 character. `GlobalState.difficulty_multiplier()`'s `0.5 * ng_plus_level` formula is the knob if NG+1 now opens too hard.
3n. **Playtest the crafting system** (see §0m, rebalanced in §0q) — open the inventory, hit the Craft tab, confirm it lists 6 recipes (3 epic + 3 mythic, not the original 9) with correct have/need material counts, that mythic ones show "Requires New Game+" and stay disabled pre-NG+, that the button otherwise disables only when short on a material, and that crafting deducts materials and produces the item correctly (including the equip-upgrade prompt if it's better than what's equipped). Then gauge whether the bumped material drop rate (`CATEGORY_WEIGHTS["material"]` 0.20) and recipe costs (`ItemDatabase.CRAFTING_RECIPES`) feel right -- still first-pass numbers.
3o. **Playtest the Skeleton Archer ranged enemy** (see §0n) — in map2, confirm it holds distance and fires a projectile instead of always closing to melee, that closing the gap yourself still triggers normal contact damage, and that the pale-green tint reads as clearly distinct from the regular white Skeleton at a glance. `enemy.gd`'s `ranged_attack_range`/`RANGED_ATTACK_COOLDOWN` and the spawner's `dragon_count` (4) are the knobs if it's too easy to kite around or too oppressive to approach.
3p. **Playtest the new "Frostbound Wastes" map end-to-end** (see §0o) — confirm map2's exit gate now leads into it instead of straight to map3, clear Frozen Zombie/Frost Drake, defeat the Frostbound Warden, confirm its own exit gate correctly opens into the existing map3, and gauge whether the 18-22 mob level range and the Warden's level-29/3185-HP-bonus/60-dmg-bonus stats feel like a fair bridge between Gate Overlord and Dragon Sovereign. `map2b.tscn`'s spawner/boss `@export` values are the knobs to retune.
